...
#if defined(NO_LIBC) && !defined(SUBRACORE)
#include <ctype.h>
#endif
#if defined(SUBRACORE)
#include "oslib/assert.h"
#include "oslib/stddef.h"
...
#include "oslib/assert.h"
#include "oslib/string.h"
#include "memory.h" //OS support functions (malloc et al)
#define MBUF_FREE dlfree
#define MBUF_REALLOC dlrealloc
#else
#include <assert.h>
#include <string.h>
#endif
...
#define abort halt
#define MG_MALLOC dlmalloc
#define MG_FREE dlfree
#define MG_CALLOC dlcalloc
#else
...
DIR *opendir(const char *dir_name) {
  ...
  if (dir_name == NULL || fs == NULL ||
      (dir = (DIR *) MG_CALLOC(1, sizeof(*dir))) == NULL) {
    return NULL;
  }
...
#if !defined(EXCLUDE_COMMON) && !defined(SUBRACORE) //-No File APIs
...
bcode_decode_lit(struct v7 *v7, struct bcode *bcode, char **ops) {
  ...
  switch (idx) {
    ...
    case BCODE_INLINE_FUNC_TYPE_TAG: {
      ...
      func->bcode = (struct bcode *) MG_CALLOC(1, sizeof(*func->bcode));
...
static struct v7_call_frame_base *create_call_frame(struct v7 *v7,
  ...
  call_frame_base = (struct v7_call_frame_base *) MG_CALLOC(1, size);
  ...
  while (r.ops < r.end && rcode == V7_OK) {
    ...
    switch (op) {
      ...
      case OP_PUSH_PROP_ITER_CTX: {
        struct prop_iter_ctx *ctx =
            (struct prop_iter_ctx *) MG_CALLOC(1, sizeof(*ctx));
...
/* TODO(mkm): use GC pool */
#if !defined(V7_NO_COMPILER)
  struct ast *a = (struct ast *) MG_MALLOC(sizeof(struct ast));
#endif
  ...
  /* init new bcode */
  bcode = (struct bcode *) MG_CALLOC(1, sizeof(*bcode));
...
struct v7 *v7_create_opt(struct v7_create_opts opts) {
  ...
  if ((v7 = (struct v7 *) MG_CALLOC(1, sizeof(*v7))) != NULL) {
#ifdef V7_STACK_SIZE
    ...
    v7->cur_dense_prop =
        (struct v7_property *) MG_CALLOC(1, sizeof(struct v7_property));
...
const char *v7_get_cstring(struct v7 *v7, v7_val_t *value) {
  ...
  if (s[size] != 0 || (size_t)strlen(s) != size) {
    return NULL;
  }
  return s;
}
...
enum v7_err v7_array_set_throwing(struct v7 *v7, val_t arr, unsigned long index,
  ...
  if (v7_is_object(arr)) {
    if (get_object_struct(arr)->attributes & V7_OBJ_DENSE_ARRAY) {
      ...
      if (abuf == NULL) {
        abuf = (struct mbuf *) MG_MALLOC(sizeof(*abuf));
...
enum v7_err v7_mk_regexp(struct v7 *v7, const char *re, size_t re_len,
  ...
  if (slre_compile(re, re_len, flags, flags_len, &p, 1) != SLRE_OK ||
    ...
    *res = mk_object(v7, v7->vals.regexp_prototype);
    rp = (struct v7_regexp *) MG_MALLOC(sizeof(*rp));
...
V7_PRIVATE struct shdata *shdata_create(const void *payload, size_t size) {
  struct shdata *ret =
      (struct shdata *) MG_CALLOC(1, sizeof(struct shdata) + size);
  shdata_retain(ret);
  if (payload != NULL) {
...
static struct gc_block *gc_new_block(struct gc_arena *a, size_t size) {
  ...
  heapusage_dont_count(1);
  b = (struct gc_block *) MG_CALLOC(1, sizeof(*b));
  heapusage_dont_count(0);
  if (b == NULL) abort();

  b->size = size;
  heapusage_dont_count(1);
  b->base = (struct gc_cell *) MG_CALLOC(a->cell_size, b->size);
  heapusage_dont_count(0);
...
_cr_iter_begin:
  ...
  switch ((enum my_fid) CR_CURR_FUNC()) {
    ...
    default:
      /* should never be here */
      fprintf(stderr, "fatal: wrong func id: %d", CR_CURR_FUNC());
      break;
  };
...
/* disabled because we broke get_lit */
    ...
    case AST_FUNC: {
      ...
      func->bcode = (struct bcode *) MG_CALLOC(1, sizeof(*bbuilder->bcode));
...
#else
#define SLRE_MALLOC dlmalloc
#define SLRE_FREE dlfree
#endif
#define SLRE_THROW(e, err_code) longjmp((e)->jmpbuf, (err_code))
...
#if defined(__cplusplus) && !defined(SUBRACORE)
  ::jmp_buf jmpbuf;
#else
  jmp_buf jmpbuf;
#endif
...
int slre_compile(const char *pat, size_t pat_len, const char *flags,
  ...
  if ((err_code = setjmp(e.jmpbuf)) != SLRE_OK) {
    SLRE_FREE(e.pstart);
    SLRE_FREE(e.prog);
    return err_code;
  }
...
static int printf_stack_line_cfunc(char *p, size_t len, v7_cfunction_t *cfunc,
  ...
  char *buf = (char *) MG_MALLOC(name_len);
...
static enum v7_err a_sort(struct v7 *v7,
  ...
  *res = v7_get_this(v7);
  ...
  arr = (val_t *) MG_MALLOC(len * sizeof(arr[0]));
    ...
    do {
      ...
      if (v7_is_callable(v7, str_func)) { /* replace function */
...

        {
          ...
          if (tmp_str_buf == NULL) {
            tmp_str_buf = (val_t *) MG_CALLOC(sizeof(val_t), V7_RE_MAX_REPL_SUB);
          }
