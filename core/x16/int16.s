...
;        int16() resets all selectors.
;
; C Prototype:
;   void _cdelc int16(unsigned char intnum, regs16_t *regs);
; 
; Example of usage:
;   regs.ax = 0x0013;
;   int16(0x10, &regs);
...
section .text
    ...
    _int16:
        ...
        jmp INT16_BASE                         ; jump to new code location
    ...
    ib: db 0x00
        ...
        mov  sp, INT16_BASE                    ; set correct stack position so we can copy back
    ...
    int16_end:                                 ; end marker (so we can copy the code)
